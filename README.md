...
- **Transactional SQLite Persistence & Centralized Schema** — Atomic client and reading deletions using explicit SQLite transactions (`conn.transaction()`), indexed lookup queries, centralized schema initialization (`create_tables`), and clean row mapping abstractions (`ClientRecord::from_row`).
- **Pinned Dependency Stability & Full Test Coverage** — Exact version pinning for downstream stability (`tzf-rs = "=1.2.0"`), robust filename sanitization (`sanitize_filename`), and comprehensive unit test suites covering API, database operations, Nominatim result parsing, utilities, and stubbed domain modules.
